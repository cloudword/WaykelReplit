...
export function BookingForm() {
  ...
  const handleMagicParse = async () => {
    ...
    setIsAiLoading(true);
    try {
      const result = await waykelApi.ai.parseBooking(magicPrompt);

      setFormData(prev => ({
        ...prev,
        ...result,
        // Ensure values are strings to avoid controlled/uncontrolled issues
        pickupLocation: result.pickupLocation || prev.pickupLocation,
        dropLocation: result.dropLocation || prev.dropLocation,
        cargoType: result.cargoType || prev.cargoType,
        weightValue: result.weightValue?.toString() || prev.weightValue,
        weightUnit: result.weightUnit || prev.weightUnit,
        requiredVehicleType: result.requiredVehicleType || prev.requiredVehicleType,
        date: result.date || prev.date,
        pickupTime: result.pickupTime || prev.pickupTime,
        budgetPrice: result.budgetPrice?.toString() || prev.budgetPrice,
      }));
      ...
      // Clear magic prompt after success
      setMagicPrompt("");
    } catch (error) {
      toast({
        title: "AI Parsing Failed",
        description: "Try being more specific or fill the form manually.",
        variant: "destructive"
      });
    } finally {
      ...
      <CardContent className="p-0">
        ...
        <div className="bg-primary/5 p-6 border-b border-primary/10">
          ...
          <div className="relative group">
            <Textarea
              placeholder="e.g., I need a truck for 5 tons of steel from Ghaziabad to Meerut for tomorrow morning at 10 AM..."
              value={magicPrompt}
              onChange={(e) => setMagicPrompt(e.target.value)}
              className="resize-none min-h-[80px] pr-28 border-primary/20 focus:border-primary bg-background/50 group-hover:bg-background transition-colors"
            />
            <Button
              type="button"
              onClick={handleMagicParse}
              disabled={isAiLoading || !magicPrompt.trim()}
              className="absolute bottom-2 right-2 h-9 px-4 gap-2 bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg"
            >
              {isAiLoading ? (
          ...
          <p className="text-[10px] text-muted-foreground mt-2 font-medium">
            Type your trip details above and let AI handle the form filling for you.
          </p>
        </div>
